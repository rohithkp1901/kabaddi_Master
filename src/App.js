...
function App() {
  ...
  return (
    <div className="App" style={{ color: 'white', textAlign: 'center',position:'relative' }}>
      <div src='./kabaddi_master.png' style={{width:'250px',position:'absolute',right:'30px',top:"-20px"}}>
        <p style={{fontSize:'35px',fontFamily:' Dancing Script, cursive'}}>KabaddiMaster</p>
   <p style={{position:'absolute',right:'10px',top:'50px'}}> _a_trave_ller</p>
      </div>
      ...
      <div style={{ fontSize: '40px',position:'absolute',top:'330px',left:'720px'}}>
          {selectedRound && <div style={{border: '1px solid white',fontWeight:'bold', padding: '10px', display: 'inline-block',background:'black'}}><span>Half  </span>{selectedRound}</div>}
        </div>
      <div style={{display:'flex'}}>
      <div>
      <span>Match Time </span>
        <div className='mtime' style={{fontSize:'90px',fontWeight:'bold',color:matchTime<=5 ? 'red':'yellow', fontFamily: 'Roboto Mono, monospace'}}>
      ...
      <div className='ctime' style={{ color: 'white', textAlign: 'center'}}>
        <Timers initialCounter={30} />
      </div>
      </div>
      
      <div className='sboard' style={{ textAlign: 'center', position:'absolute',top:'280px' }}>
        <div className='sba'>
          <Scoreboard teamA={teamAName} scoreA={scoreA} pointsAdded={pointsAddedA} doOrdie={doOrDieA} />
          <Buttons
            teamAName={teamAName}
            scoreA={scoreA}
            onAddPointA={handleAddPointA}
            onSubtractPointA={handleSubtractPointA}
            isMatchRunning={isMatchRunning}
            doordieA={handleDoOrDieA}
          />
        </div>
        <div className='sbb'>
          <Scoreboard teamB={teamBName} scoreB={scoreB} pointsAdded={pointsAddedB} doOrdie={doOrDieB} />
          <Buttons
            teamBName={teamBName}
            scoreB={scoreB}
            onAddPointB={handleAddPointB}
            onSubtractPointB={handleSubtractPointB}
            isMatchRunning={isMatchRunning}
            doordieB={handleDoOrDieB}
          />
        </div>
      </div>
